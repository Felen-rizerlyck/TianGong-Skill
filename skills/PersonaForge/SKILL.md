...
### Phase 2.5: GAN对抗验证（可选/重要人物启用）

Phase 2提炼完成后，**如果用户选择GAN模式**，启动第7个Agent作为Discriminator进行对抗验证：

#### 触发条件

用户说「GAN模式」「用GAN蒸馏」「对抗验证」时启用，或默认对重要人物（公开信息50+来源）启用。

#### Agent 7: Discriminator Prompt
...
### 1. 准确性验证
对每个心智模型，回答：
- 这个模型的claim在原始调研数据中有证据吗？（标注来源）
- 有没有相反的证据被遗漏？
- 这是原创发现还是事后归纳的通用道理？
- 区分度：这个模型是唯此人所有，还是"聪明的都这样"？
...
准确分：X/10
简洁分：X/10
综合分：X/10（准确分*0.7 + 简洁分*0.3）

### 心智模型逐个评估
[每个模型的准确分、简洁分、问题、改进建议]
...
### 结论
□ 达标（综合分≥8.0）→ 进入Phase 3
□ 需迭代 → 返回Phase 2
```

#### 迭代规则

- **判别达标**：综合分≥8.0 或 达到2轮迭代上限
- **迭代返回**：如果综合分<8.0，返回Phase 2，附带Discriminator的改进建议
- **上限控制**：Phase 2↔Discriminator最多循环2次
- **诚实边界**：2轮后仍不达标的问题，标记为「诚实边界」，继续后续流程
- **非GAN模式**：用户未选择GAN模式时，跳过Phase 2.5 Discriminator验证

---

### Phase 2.6: 提炼确认检查点

Phase 2.5完成后（GAN模式）或Phase 2完成后（默认模式），暂停展示提炼摘要给用户确认：
